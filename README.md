...
# Building/Modifying FPGA Layer and HPS
## Modifying FPGA/HPS Design
1. Open Quartus and the Platform Designer
2. In the Platform Designer
    1. Open the Qsys file in the design folder
    2. Click `Generate HDL`
    3. `Show instantiate template` can be useful for generating the boilerplate HDL for instantiating the new design
    3. Click `Finish` once complete
3. If design was changed, make modifications to main HDL project to align.
4. Open the Tcl scripts window and execute scripts generated by platform designer
    1. parameters.tcl
    2. pin_assignments.tcl
5. Compile FPGA Design

## Incorporating Design into Main Linux Build
1. Hardware Design
    1. File -> Convert Programming Files
    2. Click `Add File` and navigate to hardware/output_files. Select the sof file
    3. update the bsp layer's rbf file with the new one in the recipes-bsp/hardware folder
    4. update the u-boot qts hps handoff c files in recipes-bsp/u-boot using the cv_bsp_generator script.
        1. For convenience, these support scripts are checked out independantly and added to the PATH
        2. example call from top level o f project: `cv_bsp_generator.py --input-dir=hardware/hps_isw_handoff/hps_fpga_hps_0 --output-dir=layers/meta-de1-soc/recipes-bsp/u-boot/u-boot-socfpga/qts/`
